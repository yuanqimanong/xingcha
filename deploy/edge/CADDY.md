...
两个脚本几乎不做事，真正的活是 caddy 自己的子命令干的。它们只负责三件**忘了就出
问题、而症状指不到这里**的事：认本目录的二进制（优先于 PATH 里的——两个版本同时
在，"我明明升级了"却跑着旧的那个，是最费时的一类困惑）、把 `EDGE_HOST` 传进进程、
认准这一份 `Caddyfile` 而不是"你在哪儿敲的命令"。
...
`0.0.0.0`，那是监听地址、不是能敲的名字）。它是证书上的名字，也是你在浏览器里敲的
地址。两个脚本都从那儿读，**不另设变量**——两个来源必然有一天不一致，而那时页面上
印的地址和证书上的名字对不上。
...
两条路的结果一样：星槎监听 `127.0.0.1:8720`，**局域网上连不到**，唯一进得来的就是
本机这个 Caddy。docker 那条由 [`../linux/xc`](../linux/xc) 的 `derive_bind_addr`
强制绑回环，不受 `XINGCHA_WEB_HOST` 影响；变量的完整注释在
[`../.env.example`](../.env.example)。

`xc start` 会先看 8443 上有没有人听，没有就停下来告诉你先起网关——**网关是硬依赖**，
它没起的话后台完全进不去，而那时容器是 healthy 的，每一层单独看都正常。
...
本机一条 `edge trust` 就完了。别的设备跑 `edge ca`：它把 `root.crt` 导出到这个目录
（公钥，随便传），并印出各系统的安装命令与指纹。装完**都要重启浏览器**。
...
`Caddyfile` 末尾有一段注释掉的模板：照抄、换个端口、换个 `reverse_proxy` 目标，
然后 `edge reload`。证书与 CA 是同一份，所以**根证书仍然只在每台设备装一次**——
这就是"一台机器一个网关"而不是"每个项目自带一个"的全部意义：后者是 N 个 CA、装 N
次，那种事没人会坚持做，最后大家都在点"继续前往"，而那一档只防被动嗅听。

按**端口**分流而不是主机名：内网往往没有 DNS，而 TLS 的 SNI 也不允许装 IP。
还没起来的站点回 502，不影响别的项目。

别的项目也要把自己的端口**绑在 `127.0.0.1` 上**（docker 的话是
`ports: - "127.0.0.1:8000:8000"`）。绑到 `0.0.0.0` 就是在 TLS 旁边另开一条明文入口，
而且 docker 发布的端口走 `DOCKER-USER` 链、**绕过 ufw**——你在防火墙里写的 deny
对它无效。
...
**为什么不跨机器反代**（Caddy 在 A、星槎在 B）：那会把「Caddy → 星槎」这一跳变成
**跨网络的明文**，而浏览器里看着是 HTTPS——最容易被误当成端到端加密的拓扑。顺带还
让网关变成跨机器的硬依赖：星槎那台好着、网关那台挂了，服务就打不开。

**为什么不做成容器**：那等于"要 HTTPS 就必须先装 docker"。而没装 docker 的机器
（Windows，或干净的 Linux）正是这条路存在的理由——不该为了一个反代装一层虚拟机。
单文件版两个系统通用，配置也就只有这一份。
