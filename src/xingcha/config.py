...
class Settings(BaseSettings):
    ...
    #:
    #: 两个方向的错都是静默的。``X-Forwarded-Proto`` 决定会话 cookie 带不带
    #: ``Secure``（见 web/admin/security.py 的 cookie_secure）：无条件信任，任何能直连
    #: 应用的人都能伪造一个 https，还会污染日志里的来源 IP；配漏了，挂在 HTTPS 反代
    #: 后面的应用以为自己在 http 上，cookie 少一层保护而功能完全正常。所以默认谁都不信，
    #: 要信就显式写。
    #:
