...
async def _settings_ctx(
    ...
    *,
    ...
    抽出来是为了让 POST 失败时能就地回显：密码填错时直接渲染这一页、保留已填好的地址与
    public key，而不是跳到独立的错误页让人把地址 + 两把 key + 密码全部重填一遍——而"密码
    错"恰恰是这个表单最常见的失败。
    """
...
async def change_password(
    ...
    改完吊销所有会话，包括当前这个：改密码的场景往往正是"我怀疑密码泄漏了"，那一刻留着
    任何一个旧会话都等于没改。而"只踢别人"需要区分会话来源，多一层不必要的逻辑。
...
async def save_trace_target(
    ...
    要密码，理由和上游 key 一样：这个表单一旦被跨站提交，攻击者就得到一份持续到达的对话
    副本，后果与偷走 key 同一个量级。

    校验全部在写库之前：密码不对或地址被拒时一个字节都不改，并且就地回显这一页保留已填
    内容。endpoint 过 SSRF 守卫——它是一个"服务端会主动去打"的地址，和上游地址同类。
    """
    await guard_mutation(request, csrf_token)
    state = request.app.state.xc
...
async def activate_trace(
    ...
    这一个不要密码而改地址那个要，差别在攻击面：改地址是"把对话副本送到我指定的地方"，
    启用只能送到管理员自己早就存下来的那个地址。真正的门是选目的地那一步，这里有 CSRF
    就够了——给一个每天要用的开关加密码，结果是没人去关它。
    """
    await guard_mutation(request, csrf_token)
    state = request.app.state.xc
