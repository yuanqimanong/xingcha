...
3. **解析域名后逐个 IP 判黑**——只按域名字面判挡不住一个指向内网的域名

第 3 条到此为止：连接**没有** pin 到解析出的那些 IP，所以"校验时解析到公网、
真正连接时解析到 127.0.0.1"这个 DNS rebinding 窗口仍然存在。
:attr:`CheckedURL.addresses` 是给那一步留的位置，目前没有消费者。
"""
